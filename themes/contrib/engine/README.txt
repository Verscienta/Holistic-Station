...
engine boasts a clean HTML5 structure with extensible CSS classes and IDs for
...
engine's goal is to provide themers the building blocks needed to get their
designs up and running quickly and simply.

engine is perfect if you want a simple, smart, and flexible theme starter.
...
engine utilizes SASS for adaptive grids and layouts and general structure of the
...
engine is meant to be YOUR theme. Follow one of the two methods below and you can
rename 'engine' to another name like 'mytheme'. You're also welcome to keep
'engine'.

1. DRUSH: The provided drush install script will duplicate the engine theme
   folder and rename all appropriate files and content to the new theme name you
   provide.

  - Download and enable the engine theme: $ drush en engine
  - Set the theme as default $ drush config-set system.theme default engine
    or copy the includes/engine.drush.inc into your .drush folder.
  - Run the provided drush install script: $ drush engine-install
  ...
  - At this time, you can uninstall engine: $drush pmu engine
...
2. MANUAL: To manually change the name of the theme follow these steps BEFORE
  ...
  - Rename engine.info.yml to mytheme.info.yml
  - edit engine.info.yml and change the name, description, project (can be
    deleted), and change all other instances of 'engine' to 'mytheme'
  - Rename engine.libraries.yml to mytheme.libraries.yml
  - Rename engine.theme to mytheme.theme
  - in engine.theme, change each instance of 'engine' to 'mytheme'
  - Rename config/schema/engine.schema.yml to mytheme.schema.yml
  - Rename each file in config/install from block.block.engine_tools.yml (for
    example) to block.block.mytheme_tools.yml
  - Every file in config/install, change each instance of 'engine' to
    'mytheme'
  - In js/source/scripts.js, change each instance of 'engine' to 'mytheme'
  - In theme-settings.php, change each instance of 'engine' to 'mytheme'
  - In templates/html.html.twig, change each instance of 'engine' to 'mytheme'
  - In templates/menu-local-tasks.html.twig, change each instance of 'engine' to
    'mytheme'
  - In templates/status-messages.html.twig, change each instance of 'engine' to
    'mytheme'

  When renaming, remember the following:
  - Do not simply replace every instance of 'engine' in every file in the theme.
    Most of engine's dependencies use the word 'engine' somewhere and renaming
    these instances will cause engine to break in unpredictable ways.
  ...
  - If you don't bother renaming engine in the above locations, be advised that
    you will run into conflicts with other versions of engine on your site. If
    your site uses more than one theme based on engine, make sure at least one of
    the themes has been renamed properly!


============================
How to compile SASS in engine
============================

To use SASS and automatically compile it within your theme, please refer to "How
to Use Grunt with engine" in the documentation below.
...
- build/scripts.js & source/scripts.js
  ...
  folder. See comments in engine.libraries.yml file to enable the starter
  scripts.js file.
...
The layout used in engine is fairly similar to the Holy Grail method. It has been
...
This is how the page template is buit in engine, and it works in fluid and fixed
layout. Refer to the notes in layout.sass to see how to modify the layout.


===========================
How to Use Grunt with engine
===========================
...
Once Node.JS is installed, go to the root folder of engine and install your Grunt
packages:
...
Once installed, cd to the root folder of engine and run Grunt via the command
line:
...
Once you start using engine, you will massively change it until you reach the
point where it has nothing to do with engine anymore. Unlike Zen, engine is not
...
version of engine comes out. Always see engine as a STARTER, and as soon as you
start using it, it is not engine anymore, but your own theme.

If you didn't rename your theme, but you don't want to be notified when engine
has a new version by the update module, simply delete "project = "engine" in
engine.info.yml.
...
Thanks for using engine, and remember to use the issue queue in drupal.org for
any questions or bug reports:

http://drupal.org/project/issues/engine
